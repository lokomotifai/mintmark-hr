...
This pack reaches two of the categories Article 6 treats most strictly, because
employment data reaches both in practice.

`HEALTH` arrives through leave requests and working arrangements, and stays at
category granularity: a condition class and nothing more.

`CRIMINAL` arrives through reference checks, and stays procedural. A document
may record that a criminal record document was requested, received, or is
pending. It never records what the document contains. Text asserting that a
person committed an offence is an accusation about a person, and a synthetic
person is still a person-shaped claim.

A denied vocabulary holds each boundary and a test plants a term to prove each
list still catches one. This is not clinical data, and it makes no accusation
about anybody.

One structural choice follows from the law rather than from convenience: the
structured `leave_record.type` column carries no label at all, including its
sick-leave code. Labeling a database enum as health data would teach a detector
to flag codes, and a coverage number built that way would mean nothing.
